...
## 実験的なプロジェクト：Jevとは本質的に異なります

**FmJevは、JevのAPI形式の一部を模倣する実験です。Jevとは本質的に異なり、そのモデル構造・学習方法・推論の仕組みを再現していません。**

FmJevでは、文章生成モデルに確率の数値を構造化テキストとして生成させ、Pythonで解析・検証・正規化し、レスポンスに組み直しています。さらに質問ごとに別の `fm` プロセスを起動して順次実行します。このように余分な生成・変換による無駄な処理やオーバーヘッドが入っており、レスポンス形式を似せても、Jevのネイティブな判断の仕組みや効率性は再現できません。

確率は未校正のモデル生成値であり、confidenceも独自の式で計算しています。Jevと似た形式の応答でも、同等の精度・信頼性・速度を意味しません。本プロジェクトは実験用であり、Jevと同等の実装や、本番運用向けの代替ではありません。
